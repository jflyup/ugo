...
type PrrSender struct {
	bytesSentSinceLoss      uint32
	bytesDeliveredSinceLoss uint32
	ackCountSinceLoss       uint32
	bytesInFlightBeforeLoss uint32
}

// OnPacketSent should be called after a packet was sent
func (p *PrrSender) OnPacketSent(sentBytes uint32) {
...
// recovery.
func (p *PrrSender) OnPacketLost(bytesInFlight uint32) {
...
func (p *PrrSender) OnPacketAcked(ackedBytes uint32) {
	p.bytesDeliveredSinceLoss += ackedBytes
	p.ackCountSinceLoss++
}

// TimeUntilSend calculates the time until a packet can be sent
func (p *PrrSender) TimeUntilSend(congestionWindow, bytesInFlight, slowstartThreshold uint32) time.Duration {
	// Return QuicTime::Zero In order to ensure limited transmit always works.
	if p.bytesSentSinceLoss == 0 || bytesInFlight < 1460 {
		return 0
	}
	if congestionWindow > bytesInFlight {
		...
		if p.bytesDeliveredSinceLoss+p.ackCountSinceLoss*1460 <= p.bytesSentSinceLoss {
