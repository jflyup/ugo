...
func (c *cubicSender) TimeUntilSend(now time.Time, bytesInFlight uint32) time.Duration {
	if c.InRecovery() {
...
func (c *cubicSender) OnCongestionEvent(rttUpdated bool, bytesInFlight uint32, ackedPackets PacketVector, lostPackets PacketVector) {
	if rttUpdated && c.InSlowStart() && c.hybridSlowStart.ShouldExitSlowStart(c.rttStats.LatestRTT(), c.rttStats.MinRTT(), c.GetCongestionWindow()/1460) {
		c.ExitSlowstart()
	}
...
func (c *cubicSender) isCwndLimited(bytesInFlight uint32) bool {
